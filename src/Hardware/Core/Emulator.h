...
class Emulator {
   public:
    ...
    bool InitFromMemory(std::span<const uint8_t> data, const std::string& name, std::string& errorMsg);
    void Reset();
   ...
   private:
    ...
    int ProcessBatch(int count, bool hooks, bool hasBreakpoints, bool hasComplex, int& batchInstructions, int& stopCode);
