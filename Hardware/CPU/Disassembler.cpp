...
DisassembledInstruction Disassembler::Disassemble(const Bus& bus,
                                                  uint16_t addr) {
    DisassembledInstruction di;
    di.size = 1;

    std::stringstream ssAddr;
    ssAddr << std::hex << std::uppercase << std::setw(4) << std::setfill('0')
           << addr;
    ...
    ssBytes << std::hex << std::uppercase << std::setw(2) << std::setfill('0')
            << (int)opcode << " ";
    ...
    switch (info.mode) {
        ...
        case AddressingMode::Relative: {
            int8_t offset = (int8_t)bus.ReadDirect(addr + 1);
        ...
        case AddressingMode::ZeroPageRelative: {
            uint8_t zp = bus.ReadDirect(addr + 1);
            int8_t rel = (int8_t)bus.ReadDirect(addr + 2);
