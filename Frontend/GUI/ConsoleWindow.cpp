...
void DrawConsoleWindow(AppState& state, ImVec2 work_pos, ImVec2 work_size, float top_section_height,
    ...
    if (ImGui::IsWindowFocused()) {
        ...
        if (ImGui::IsKeyPressed(ImGuiKey_Backspace)) state.emulator.InjectKey(0x7F);
    ...
    // Copy Output Modal
    if (ImGui::BeginPopupModal("CopyConsoleOutput", NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
        static std::string fullText;
        if (fullText.empty() || fullText.length() < 10) {
            fullText.clear();
            for (const auto& line : Console::GetLines()) fullText += line + "\n";
