...
### 5. Integrated IDE (Code Editor)

The `IDEWindow` is a fully integrated development environment built directly into the emulator. It allows you to write, compile, and run C and 6502 Assembly code without ever leaving the application.

#### Editor Features
- **Syntax Highlighting**: Supports C/C++ and full CC65 Assembly syntax (including macros, directives, and 65C02 opcodes).
- **Auto-Indentation & Theming**: Built-in dark theme and smart code indentation.
- **File Management**: Open, Save, and Save As capabilities directly linked to the host file system.

#### Compiler Integration
- **Compile & Run**: Automatically saves the current file, compiles it using the embedded `CompilerFrontend` (which invokes `cc65`), and injects the resulting binary directly into the emulator's memory space. 
- **Debug Symbols**: Automatically parses and loads the `.dbg` file generated by the compiler, linking your source code to the disassembler and enabling source-level debugging in real-time.
- **Compile & Export**: Compiles the code and prompts you to save the generated `.bin` and `.dbg` files anywhere on your system for external use.

> [!NOTE]
> The IDE relies on the host system's `cc65` installation to compile the code. In WebAssembly builds, the IDE functionality is disabled.
